        ...
        <div class="container">
                    ...
                    <div class="map-grid">
                        <?php foreach($mapas as $m): ?>
                            <a class="map-btn" href="index.php?controlador=team&amp;action=home&amp;mapa=<?php echo urlencode($m); ?>">
                                <img src="imagenes/mapas/<?php echo htmlspecialchars($m); ?>.png" alt="<?php echo htmlspecialchars($m); ?>" class="map-img" loading="lazy" decoding="async">
            ...
            <?php else: ?>
                ...
                <form action="index.php" method="get" id="comp-form">
                    ...
                    <!-- Mandamos siempre recomendar=1 para que cualquier submit muestre las recomendaciones -->
                    <input type="hidden" name="recomendar" value="1">

                    <!-- Paso 2: selector de agentes por rol -->
                    <div class="step-block">
                        <div class="step-block-header">
                        ...
                        <?php
                            // Calcular qué roles están ya cubiertos por los seleccionados
                            $roles_sel = [];
                            ...
                            // Descripciones didácticas del rol (aparecen como tooltip)
                            $roles_tip = [
                                'Duelist'    => 'Entry fragger: entra primero a ganar espacios y primeras bajas.',
                                'Initiator'  => 'Abre sites con información, flashes y stuns para romper holds.',
                                'Controller' => 'Bloquea visibilidad con humos y divide el mapa para el equipo.',
                                'Sentinel'   => 'Defiende flancos, da info y ancla los sites con traps y utility.',
                            ];
                        ?>
                        <ul class="role-pills" id="role-pills" aria-label="Roles cubiertos en el equipo">
                            ...
                            <div class="step-block">
                                ...
                                <ul class="lineup-agents-row-list">
                                    <?php foreach($lista as $a):
                                        $ruta_img = $a['nombre'] === 'KAY/O'
                                            ? 'imagenes/agentes/KAY/O.png'
                                            : 'imagenes/agentes/' . rawurlencode($a['nombre']) . '.png';
                                    ?>
                                        <li>
                ...
                <?php if(isset($resultado) && $resultado !== null): ?>
                    <div class="recommendation-result reveal-zoom" id="recommendation-result">
                        ...
                        <div class="rec-body">
                                ...
                                <section class="rec-block">
                                    ...
                                    <ul class="lineup-agents-row-list">
                                        <?php foreach($resultado['seleccionados'] as $s):
                                            $ruta_img_s = $s['nombre'] === 'KAY/O'
                                                ? 'imagenes/agentes/KAY/O.png'
                                                : 'imagenes/agentes/' . rawurlencode($s['nombre']) . '.png';
                                        ?>
                                            <li>
                                                <div class="lineup-agent-chip role-<?php echo strtolower($s['rol']); ?> is-active" data-rol="<?php echo htmlspecialchars($s['rol']); ?>" title="<?php echo htmlspecialchars(mini_descripcion_agente_mapa($s, $mapa)); ?>">
                            ...
                            <?php else: ?>
                                <p class="empty-hint">No has marcado ningún agente. Las recomendaciones son solo por meta del mapa.</p>
                            <?php endif; ?>

                            <!-- === SECCIONES FALTANTES (exactamente 5 − seleccionados) === -->
                                        ...
                                        <?php else: ?>
                                            <ul class="lineup-agents-row-list">
                                                <?php foreach($opciones as $i => $a):
                                                    $ruta_img_a = $a['nombre'] === 'KAY/O'
                                                        ? 'imagenes/agentes/KAY/O.png'
                                                        : 'imagenes/agentes/' . rawurlencode($a['nombre']) . '.png';
                                                ?>
                                                    <li>
                                                        <div class="lineup-agent-chip role-<?php echo strtolower($a['rol']); ?>" data-rol="<?php echo htmlspecialchars($a['rol']); ?>" title="#<?php echo $i + 1; ?> · <?php echo htmlspecialchars(mini_descripcion_agente_mapa($a, $mapa)); ?>">
                            ...
                            <!-- === RATING DEL EQUIPO COMPLETO (5 seleccionados) === -->
                                ...
                                <section class="rec-block team-rating-block">
                                    ...
                                    <div class="team-rating-grid">
                                        ...
                                        <ul class="team-rating-roles">
                                            <?php
                                                $roles_info = [
                                                    'Duelist'    => ['es' => 'Duelistas',    'ok' => $tr['conteos']['Duelist'] >= 1 && $tr['conteos']['Duelist'] <= 2, 'max' => 2],
                                                    'Initiator'  => ['es' => 'Iniciadores',  'ok' => $tr['conteos']['Initiator'] >= 1,  'max' => null],
                                                    'Controller' => ['es' => 'Controladores','ok' => $tr['conteos']['Controller'] >= 1, 'max' => null],
                                                    'Sentinel'   => ['es' => 'Centinelas',   'ok' => $tr['conteos']['Sentinel'] >= 1,   'max' => null],
                                                ];
                                            ?>
                                            <?php foreach($roles_info as $r => $info): ?>
                                                <li class="team-rating-role role-<?php echo strtolower($r); ?> <?php echo $info['ok'] ? 'is-ok' : 'is-off'; ?>">
                                                    <span class="team-rating-role-count"><?php echo (int)$tr['conteos'][$r]; ?><?php if($info['max']): ?> / <?php echo $info['max']; ?><?php endif; ?></span>
                                    ...
                                    <p class="team-rating-msg <?php echo $tr['balance_ok'] ? 'is-ok' : 'is-off'; ?>">
                                        ...
                                        <?php else: ?>
                                            ⚠ Composición desbalanceada — revisa las reglas (máx 2 Duelistas; mín 1 Iniciador, Controlador y Centinela).
                                        <?php endif; ?>
                                    </p>
                                </section>
                            <?php endif; ?>
